...
export function useLimits(userId = null) {
  ...
  const checkTokenLimit = async (additionalTokens) => {
    // O backend usa o userId do token JWT, não precisa passar userId
    return await limitsAPI.checkTokenLimit(additionalTokens);
  };

  const checkSessionLimit = async () => {
    // O backend usa o userId do token JWT, não precisa passar userId
    return await limitsAPI.checkSessionLimit();
  };
